...
;=====================================================
    ...
    MOV     WR4, #0345H
    MOV     CAN_IDH, WR4    ;设置默认 CAN ID
    MOV     WR4, #06789H
    MOV     CAN_IDL, WR4    ;设置默认 CAN ID
...
;   CanID = (((u32)buffer[1] << 24) + ((u32)buffer[2] << 16) + ((u32)buffer[3] << 8) + buffer[4]) >> 3;
    ...
    ANL     WR0, #1FFFH
    MOV     CAN_IDH, WR0  ;解析 CAN ID 高13位
    MOV     CAN_IDL, WR2  ;解析 CAN ID 低16位
...
F_CAN_SendMsg:
    MOV     A, #TX_BUF0
    MOV     B, #88H        ;bit7: 标准帧(0)/扩展帧(1), bit6: 数据帧(0)/远程帧(1), bit3~bit0: 数据长度(DLC)
    CALL    F_CAN_WriteReg
...
F_CAN_Init:
    ...
    MOV     P_SW1, #0x10      ;端口切换(CAN_Rx,CAN_Tx) 0x00:P0.0,P0.1  0x10:P5.0,P5.1  0x20:P4.2,P4.5  0x30:P7.0,P7.1
